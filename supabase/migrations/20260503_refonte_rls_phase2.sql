...
-- INSERT : direction + commercial (seller ne s'auto-encaisse pas)
CREATE POLICY sales_insert ON sales FOR INSERT WITH CHECK (
  company_id = get_my_company_id()
  AND get_my_role() IN ('owner', 'manager', 'seller', 'field_agent')
);

-- UPDATE : direction + seller sur ses propres brouillons uniquement
...
-- Cashier est le rôle principal d'encaissement
...
-- Tous les rôles opérationnels voient le catalogue
-- Le masquage de purchase_price est géré au niveau application
...
-- INSERT via trigger uniquement (create_stock_entry_for_product)
...
-- Approbation / rejet : direction uniquement
...
CREATE POLICY customers_select ON customers FOR SELECT TO authenticated USING (
  company_id = get_my_company_id()
  AND get_my_role() IN (
    'owner', 'manager', 'seller', 'field_agent', 'accountant'
  )
);
...
CREATE POLICY suppliers_tenant_select ON suppliers FOR SELECT USING (
  company_id = get_my_company_id()
  AND get_my_role() IN (
    'owner', 'manager', 'warehouse', 'accountant'
  )
);
...
-- ════════════════════════════════════════════════════════════════════════════
-- TABLE : cash_entries + cash_closings
-- ════════════════════════════════════════════════════════════════════════════
...
-- cashier peut créer/mettre à jour sa clôture de session
...
-- ════════════════════════════════════════════════════════════════════════════
-- TABLE : sale_returns + sale_return_items
-- ════════════════════════════════════════════════════════════════════════════
...
-- seller peut initier, warehouse voit pour confirmer la réception physique
CREATE POLICY "sale_returns_insert" ON sale_returns FOR INSERT WITH CHECK (
  company_id = get_my_company_id()
  AND get_my_role() IN ('owner', 'manager', 'seller')
);

-- Validation / annulation : direction uniquement
...
-- cashier peut marquer une échéance comme payée
...
-- owner et manager (de la même company) voient les utilisateurs
...
-- owner peut modifier n'importe quel utilisateur de sa company
CREATE POLICY users_update_manager ON users FOR UPDATE USING (
  company_id = get_my_company_id()
  AND get_my_role() = 'owner'
);

-- Chacun peut modifier son propre profil (full_name, push_subscription)
...
-- ════════════════════════════════════════════════════════════════════════════
-- TABLE : boutiques
-- (inchangé — owner/manager gèrent, tous les authentifiés lisent)
-- ════════════════════════════════════════════════════════════════════════════
